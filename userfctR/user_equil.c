...
void user_equil_fxe(MbsData *mbs_data, double* f)  
{
    ...
    f[1] = mbs_data->user_model->SteeringAssembly.delta_left + 0.0; // 0.2 * M_PI / 180; // toe-in - | toe-out +
    f[2] = mbs_data->user_model->SteeringAssembly.delta_right - 0.0; // 0.2 * M_PI / 180; // toe-in + | toe-out -
}
